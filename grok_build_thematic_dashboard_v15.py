#!/usr/bin/env python3
"""
Grok Build v15 - Thematic Portfolios Dashboard
- Loads expanded FinTwit accounts (58) from CSV with equal 1.0x weighting
- All previous features preserved and cleaned
"""
...
# ============================================================
# LOAD FINTWIT ACCOUNTS FROM CSV (expanded list - equal 1.0x)
# ============================================================
def load_fintwit_accounts():
    """Load FinTwit accounts from CSV. Falls back to hardcoded list if file missing."""
    csv_paths = [
        Path("FinTwits_X_Accounts_8.12.2026.csv"),
        Path("/home/workdir/artifacts/FinTwits_X_Accounts_8.12.2026.csv"),
        Path(__file__).parent / "FinTwits_X_Accounts_8.12.2026.csv",
    ]
        ...
        if p.exists():
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    field = str(row.get("Field", "General Markets")).strip()
                    # Parse weight like "1.0x" → 1.0
                    weight_str = str(row.get("Weight", "1.0x")).replace("x", "").strip()
                    try:
                        weight = float(weight_str)
                    except:
                        weight = 1.0
                    accounts[handle] = {"field": field, "weight": weight}
                return accounts
...
FINTWIT_ACCOUNTS = load_fintwit_accounts()
...
with tab3:
    ...
    st.caption(f"Source: FinTwits_X_Accounts_8.12.2026.csv  •  {len(FINTWIT_ACCOUNTS)} total accounts")
